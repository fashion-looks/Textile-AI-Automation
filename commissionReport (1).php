...
$commissionType = trim($_POST['commissionType']);
...
$url = "".$serverurlapi."General/commissionReportAPI.php";
...
logger ('RESPONSE RETURN FROM COMMISSION REPORT API: '.$result);
...
if($_POST['action']=='approveall'){
	...
	foreach($_POST['acknowledgmentchecksingle'] as $AcknowledgementNumber){
			$AcknowledgementNumber = explode('~',$AcknowledgementNumber); 
		$ackJson.= '{
				"AcknowledgementNumber":"'.$AcknowledgementNumber[0].'",
				"commissionType":"'.$AcknowledgementNumber[1].'"
			},';
	}
...
<div class="hk-wrapper hk-vertical-nav hk-nav-toggle">
  ...
  <div class="hk-pg-wrapper"  style="">
      ...
      <form action="" method="POST" autocomplete="nope" id="exportfrm" />
        ...
        <div class="col-md-2">
          ...
          <select name="commissionType" class="form-control " >
            ...
            <option value="A" <?php if($_POST["commissionType"]=="A"){ echo "selected"; } ?>>Acceptance</option>
            <option value="D" <?php if($_POST["commissionType"]=="D"){ echo "selected"; } ?>>Digitization</option>
            <option value="I" <?php if($_POST["commissionType"]=="I"){ echo "selected"; } ?>>Incentive</option>
          </select>
        ...
        <div class="col-md-2">
          ...
          <input type="submit" name="Search" class="btn btn-success" onClick="processBar();" value="Search" />
          </div>
        </div>
    ...
    if(isset($accountData['commissionData'])){                    
    $no=1;
    foreach($accountData['commissionData'] as $resultList){

    ?>
            <tr class="uyt hgte">
              <td><input type="checkbox" style=" opacity:1; cursor: pointer; height: 17px; width: 50px; margin-top: 3px;" value="<?php echo $resultList['AckNumber'].'~'.$resultList['CommissionType']; ?>" name="acknowledgmentchecksingle[]"  class="deleteack"/></td>
              <td><?php echo $resultList['AckNumber']; ?></td>
              <td><?php  echo $resultList['ProductType']; ?></td>
              <td><?php echo $resultList['BranchCode']; ?></td>
			  <td><?php echo round(substr($resultList['Commission'],1)); ?></td>
        <td><?php if($resultList['CommissionType'] == 'A'){ echo 'Acceptance'; } if($resultList['CommissionType'] == 'D'){ echo 'Digitization'; } if($resultList['CommissionType'] == 'I'){ echo 'Incentive'; } ?></td>
              <td><?php echo date('d-m-Y',strtotime($resultList['DateAdded'])); ?></td>
             <!-- <td>


                <div id="hide<?php echo $resultList['AckNumber']; ?>" onClick="funccheck(<?php echo $no; ?>,'<?php echo $resultList['AckNumber']; ?>','<?php echo $resultList['CommissionType']; ?>');" style="cursor: pointer;color: white;background: #2f9e41;border: 1px solid #2f9e41;border-radius: 4px;padding: 3px 0px; <?php  if($resultList['CommissionFlag'] == 0){ ?>display: block; <?php }else{ ?>display: none;<?php } ?>">Approve</div>
             
                <div id="show<?php echo $resultList['AckNumber']; ?>" style="color: white;font-size: 13px;background: #1f7140;width: fit-content;padding: 1px 5px;border-radius: 50%;display: block;margin: auto; <?php  if($resultList['CommissionFlag'] == 0){ ?>display: none; <?php }else{ ?>display: block;<?php } ?>"><i class="fa fa-check"></i></div>
...
function funccheck(no,ackno,commissionType){
	var confirmed = confirm("Do you want to Approve Acknowledgement no "+ackno);
	if(confirmed){
		$('#hide'+ackno).load('frmaction.php?act=commissionApprove&ackno='+ackno+'&commissionType='+commissionType);
