...
$url = $serverurlapi."General/billGenerationAPI.php";
logger($InfoMessage." URL for API - ".$url); 
...
$url = $serverurlapi."General/addBillVoucherAPI.php";
logger($InfoMessage." URL for API - ".$url); 
logger($InfoMessage." Value for API - ".$searching); 
$resultData = postCurlData($url,$jsonData);
// $billReportData = json_decode($resultData);
...
<div class="hk-wrapper hk-vertical-nav hk-nav-toggle">
  ...
  <div class="hk-pg-wrapper"  style="background-image: url(../html/dist/img/Religare-Dashboard-BG.JPG);">
    ...
    <form action="" method="POST" autocomplete="nope" id="exportfrm" />
      ...
      ?>
            <option value="<?php echo $resultList['Id']; ?>"  <?php if($_POST['billPeriod'] == $resultList['Id']){ echo "selected"; } if($resultList['Status'] == 1){ echo "disabled"; } ?>><?php echo date('d/m/Y',strtotime($resultList['FromDate']))." - ".date('d/m/Y',strtotime($resultList['ToDate'])); ?></option>
      <?php } } } ?>
            </select>
        ...
        <div class="col-md-1">
          ...
          <input type="button" name="Generate" id="Generate" class="btn btn-success" onClick="searchFunc('generateaction');" value="Generate Bill" />
          </div>
        </div>
        </div>
